...
def configure_routes(app):
    ...
    def historico():
        ...
        # Busca histórico com dados completos da roupa, ordenado do mais recente para o mais antigo
        cursor.execute("""
            SELECT 
                h.id,
                h.data_uso,
        ...
        # Converte resultado para lista de dicionários para enviar como JSON
        historico_dados = []
        for row in cursor.fetchall():
            # Formata a data para exibição amigável
            data_formatada = datetime.strptime(row['data_uso'], "%Y-%m-%d %H:%M:%S").strftime("%d/%m/%Y às %H:%M")
            historico_dados.append({
                "id": row['id'],
                "data_uso": data_formatada,
                "nome": row['nome'],
                "tipo": row['tipo'],
                "cor": row['cor'],
                "ocasiao": row['ocasiao'],
